...
class TypescriptData(Lang):
    def __init__(self):
        self.PARSER = Parser()
        self.PARSER.set_language(Language('./vendors/tree_sitter_so/language.so', 'typescript'))
    ...
    def test_file_filter(self, filename):
        return (filename.endswith('.ts') or filename.endswith('.js')) and (
            ...
            not re.search(r'.+spec\.ts', filename, re.I) is None or
            not re.search(r'.+test\.ts', filename, re.I) is None or
            ...
            not re.search(r'test_.+', filename, re.I) is None or
            not re.search(r'.+-test', filename, re.I) is None or
