...
class CPPData(Lang):
    def __init__(self):
        self.PARSER = Parser()
        self.PARSER.set_language(Language('./vendors/tree_sitter_so/language.so', 'cpp'))

    def test_file_filter(self, filename):
        return (filename.endswith('.cpp') ) and (
    ...
    def src_file_filter(self, filename):
        return filename.endswith('.cpp') and not self.test_file_filter(filename)
