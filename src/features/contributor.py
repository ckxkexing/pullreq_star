...
def social_strength(repo_id, pr_id, months_back=3):
    ...
    return {
        "social_strength": len(linked_integrators) / core_team_size
        if core_team_size
        else 0
    }
...
def contrib_perc_commit(repo_id, pr_id):
    ...
    return {
        "contrib_perc_commit": contrib_commit_num / all_commit_num
        if all_commit_num
        else 0
    }
...
def prior_interaction(repo_id, pr_id, months_back=3):
    ...
    return {
        "prior_interaction": prior_interaction_issue_events()
